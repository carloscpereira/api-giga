...
export default class CriaContratoService {
    static async execute({ formValidation: body, sequelize, transaction, alterarVinculo = true, operadora = 'idental' }) {
        ...
        try {
            ...
            `, {
                        ...
                        const [
                            ...
                            'SELECT * FROM cn_regravigenciacontrato WHERE rvc_ds_vigencia_contrato ILIKE :vigencia LIMIT 1', { replacements: { vigencia: body.PrazoVigencia }, type: QueryTypes.SELECT, transaction: t }
                        );
        ...
        `, {
                        ...
                        const grupoFamiliar = await contrato.createGrupofamiliar({ responsavelgrupoid: responsavelFinanceiro.id }, { transaction: t });
              ...
              `, {
                        ...
                        if (modPagamento.id === '7' && !body.Pagamentos) {
                            ...
                            await Parcela.update({
                                ...
                                nossonumero: boleto.numero.substring(3),
                            }, {
                                where: {
