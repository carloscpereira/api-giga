...
export default async (req, res, next) => {
  try {
    const schema = Yup.object().shape({
      descricao: Yup.string().min(1).max(60),
      modalidadepagamentoid: Yup.number().integer(),
      ...
      tdc_in_documento_cobranca: Yup.boolean(),
      tdc_in_bloqueio_cadastro: Yup.boolean(),
      nu_prazo_inadimplencia_cobranca: Yup.number().integer(),
