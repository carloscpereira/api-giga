...
class ParcelaController {
    ...
    async pause(req, res) {
        try {
            ...
            const parcela = await ParcelaSeq.update({ paused_at: new Date(), pcl_in_pause: true }, { where: { id }, returning: true });
    ...
    async start(req, res) {
        try {
            ...
            const parcela = await ParcelaSeq.update({ paused_at: null, pcl_in_pause: false }, { where: { id }, returning: true });
            ...
            const schema = Yup.object().shape({
                ...
                validadecartao: Yup.string()
                    ...
                    .matches(/^(0?[1-9]|1[012])[\/\-]\d{4}$/gim, {
                        message: 'Invalid date format. Valid format for this field is MM/YYYY where the month must be between 1 and 12 and year with four digits',
