...
class RedeCredenciadaController {
 ...
 FROM cn_contrato cc
          ...
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS telefones
                              FROM (SELECT *
                                    FROM sp_telefone
                                    WHERE pf.id = sp_telefone.dadosid
                                      AND sp_telefone.tipotelefoneid IS NOT NULL
                                      AND sp_telefone.tipotelefoneid NOT IN (3, 4, 5, 7)) d) telefones
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS enderecos
                              FROM (SELECT sp_endereco.*, sp_tipoendereco.descricao as "tipo_endereco"
                                    FROM sp_endereco
                                             INNER JOIN sp_tipoendereco ON sp_tipoendereco.id = sp_endereco.tipoenderecoid
                                    WHERE pf.id = sp_endereco.dadosid) d) enderecos
          ...
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS planos
                              FROM (SELECT DISTINCT cnpr.descricao   as "plano",
                                                    cnpr.registroans as "resgistro_ans",
                                                    cs.descricao     as "segmentacao_assistencial",
                                                    ca.descricao     as "area_cobertura",
                                                    tco.descricao    as "tipo_contratacao"
                                    FROM cn_dentistaplano dtp
                                             LEFT JOIN cn_produto cnpr
                                                       ON dtp.planoid = cnpr.planoid AND dtp.versaoplanoid = cnpr.versaoid
                                             ...
                                             LEFT JOIN cn_tipocontratacao tco on cnpr.tipocontratacaoid = tco.id
 ...
 FROM cn_contrato cc
          ...
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS telefones
                              FROM (SELECT *
                                    FROM sp_telefone
                                    WHERE pj.id = sp_telefone.dadosid
                                      AND sp_telefone.tipotelefoneid IS NOT NULL
                                      AND sp_telefone.tipotelefoneid NOT IN (3, 4, 5, 7)) d) telefones
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS enderecos
          ...
          LEFT JOIN LATERAL (SELECT pav.dadocampo as cro
                             ...
                             WHERE pav.pessoaid = pj.id
                               AND pav.vinculoid = 33
                               AND sc.descricaocampo ILIKE '%registro do conselho clínica') cro ON TRUE
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS planos
                              FROM (SELECT DISTINCT cnpr.descricao   as "plano",
                                                    cnpr.registroans as "resgistro_ans",
                                                    cs.descricao     as "segmentacao_assistencial",
                                    ...
                                    FROM cn_clinicaplano clp
                                             LEFT JOIN cn_produto cnpr
                                                       ON clp.planoid = cnpr.planoid AND clp.versaoplanoid = cnpr.versaoid
                                             LEFT JOIN cn_segmentacaoassistencial cs on cnpr.segmentacaoassistencialid = cs.id
                                             LEFT JOIN cn_areacobertura ca on cnpr.areacoberturaid = ca.id
                                             LEFT JOIN cn_tipocontratacao tco on cnpr.tipocontratacaoid = tco.id
  ...
  async show(req, res) {
    try {
      ...
      FROM cn_contrato cc
          ...
          CROSS JOIN LATERAL (SELECT array_to_json(array_agg(row_to_json(d))) AS planos
                                      FROM (SELECT DISTINCT
                                              cnpr.descricao, cnpr.registroans, cs.descricao, ca.descricao, tco.descricao    as "tipo_contratacao"
                                            FROM cn_clinicaplano clp
