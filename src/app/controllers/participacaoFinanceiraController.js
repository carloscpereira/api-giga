...
class ParticipacaoFinanceiraController {
  async index(req, res) {
    ...
    const participacoesFinanceira = await ParticipacaoFinanceira.findAll(criteria);
    return res.json({ error: null, data: participacoesFinanceira });
  }

  async show(req, res) {
    const { id } = req.params;

    const participacaoFinanceira = await ParticipacaoFinanceira.findByPk(id);

    return res.json({ error: null, data: participacaoFinanceira });
  }
...
export default new ParticipacaoFinanceiraController();
