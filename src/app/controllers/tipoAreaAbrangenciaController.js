...
class TipoAreaAbrangenciaController {
  async index(req, res) {
    ...
    const tiposAbrangencia = await TipoAreaAbrangencia.findAll(criteria);
    return res.json({ error: null, data: tiposAbrangencia });
  }

  async show(req, res) {
    const { id } = req.params;

    const tipoAbrangencia = await TipoAreaAbrangencia.findByPk(id);

    return res.json({ error: null, data: tipoAbrangencia });
  }
...
export default new TipoAreaAbrangenciaController();
